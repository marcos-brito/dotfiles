...
local function worker(user_args)
	local calendar_themes = {
		...
		naughty = {
			...
			focus_date_bg = beautiful.yellowcolor,
			focus_date_fg = beautiful.bg_normal,
			weekend_day_bg = beautiful.bg_normal,
			weekday_fg = beautiful.fg,
			header_fg = beautiful.yellowcolor,
		...
		if popup.visible then
			...
			elseif placement == "top_right" then
				awful.placement.top_right(
					popup,
					{ margins = { top = 45, right = 65 }, parent = awful.screen.focused() }
				)
